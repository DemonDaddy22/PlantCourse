...
public class CandyDAOImpl implements CandyDAO {
    ...
    private static final String FIND_CANDY_BY_DELIVERY = "SELECT c.* FROM cady_delivery as cd JOIN candy AS c on c.id = cd.candy_id WHERE cd.delivery_id = " + DELIVERY_ID;

    private static final RowMapper<CandyData> candyDataRowMapper = new BeanPropertyRowMapper<>(CandyData.class);

    @Override
    public List<CandyData> getCandies() {
